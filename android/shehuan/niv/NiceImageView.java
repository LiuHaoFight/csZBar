...
public class NiceImageView extends AppCompatImageView {
  ...
  public NiceImageView(Context context) { this(context, null); }
  ...
  public NiceImageView(Context context, @Nullable AttributeSet attrs,
    ...
    TypedArray ta =
        context.obtainStyledAttributes(attrs, R.styleable.NiceImageView, 0, 0);
  ...
  protected void onDraw(Canvas canvas) {
    ...
    if (!isCoverSrc) {
      float sx =
          1.0f * (width - 2 * borderWidth - 2 * innerBorderWidth) / width;
      float sy =
          1.0f * (height - 2 * borderWidth - 2 * innerBorderWidth) / height;
  ...
  private void drawBorders(Canvas canvas) {
    if (isCircle) {
      if (borderWidth > 0) {
        drawCircleBorder(canvas, borderWidth, borderColor,
      ...
      if (borderWidth > 0) {
        drawRectFBorder(canvas, borderWidth, borderColor, borderRectF,
                        borderRadii);
      }
    }
  }

  private void drawCircleBorder(Canvas canvas, int borderWidth, int borderColor,
                                float radius) {
  ...
  private void drawRectFBorder(Canvas canvas, int borderWidth, int borderColor,
                               RectF rectF, float[] radii) {
    initBorderPaint(borderWidth, borderColor);
  ...
  private void initBorderRectF() {
    if (!isCircle) {
      borderRectF.set(borderWidth / 2.0f, borderWidth / 2.0f,
  ...
  private void initSrcRectF() {
    if (isCircle) {
      ...
      srcRectF.set(width / 2.0f - radius, height / 2.0f - radius,
                   width / 2.0f + radius, height / 2.0f + radius);
    } else {
  ...
  public void setCornerBottomRightRadius(int cornerBottomRightRadius) {
    this.cornerBottomRightRadius =
        Utils.dp2px(context, cornerBottomRightRadius);
    calculateRadiiAndRectF(true);
  }
